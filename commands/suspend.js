...
exports.run = async (client, message, args) => {
    ...
    let fireResponse;
    try {
        fireResponse = await roblox.setRank(Number(process.env.groupId), id, 22);
    } catch (err) {
        console.log(chalk.red('An error occured when running the command: ' + err));
        return message.channel.send({embed: {
            color: 16733013,
    ...
    message.channel.send({embed: {
        color: 0x00FF0F,
        description: `**Success!** Suspended ${username}`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        }
    ...
    logchannel.send({embed: {
        color: 0x00FF0F,
        description: `>>> :small_orange_diamond: ${message.member.nickname} has suspended ${username}. 
         :small_orange_diamond: Original Rank: ${rankNameInGroup}.`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        },
